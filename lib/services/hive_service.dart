...
class HiveService {
  ...
  Future<void> saveCodeSnippets(List<CodeSnippet> snippets) async {
    final data = snippets.map((s) => s.toJson()).toList();
    await _box.put(_codeSnippetKey, jsonEncode(data));
  }

  List<CodeSnippet> loadCodeSnippets() {
    final raw = _box.get(_codeSnippetKey);
    if (raw == null) return [];
    final list = jsonDecode(raw) as List;
    return list
  ...
  Future<void> saveDailyQuotes(List<DailyQuote> quotes) async {
    final data = quotes.map((q) => q.toJson()).toList();
    await _box.put(_dailyQuoteKey, jsonEncode(data));
  }

  List<DailyQuote> loadDailyQuotes() {
    final raw = _box.get(_dailyQuoteKey);
    if (raw == null) return [];
    final list = jsonDecode(raw) as List;
    return list
  ...
  Future<void> saveReadingItems(List<ReadingItem> items) async {
    final data = items.map((i) => i.toJson()).toList();
    await _box.put(_readingKey, jsonEncode(data));
  }

  List<ReadingItem> loadReadingItems() {
    final raw = _box.get(_readingKey);
    if (raw == null) return [];
    final list = jsonDecode(raw) as List;
    return list
